...
fn constructor_bounds() {
    assert!(std::panic::catch_unwind(|| BarrettModulus::<u32>::new(0)).is_err());
    assert!(std::panic::catch_unwind(|| BarrettModulus::<u32>::new(1)).is_err());

    let limit = (1u32) << (u32::BITS - 2);
    assert!(BarrettModulus::<u32>::try_new(limit - 1).is_some());
    assert!(BarrettModulus::<u32>::try_new(limit).is_none());
}
...
fn mul_slice_ops() {
    ...
    for &len in &[0usize, 1, 3, 7, 8, 15, 16, 17, 31, 33, 64, 65] {
        ...
        let mut assign = a.clone();
        m.reduce_scalar_mul_slice_assign(&mut assign, scalar);
        assert_eq!(assign, expected_scalar, "scalar_mul_slice_assign len={len}");
        let mut to = vec![0; len];
        m.reduce_scalar_mul_slice_to(&a, scalar, &mut to);
        ...
        let expected_asc: Vec<u32> = c
            ...
            .collect();
        let mut acc = c.clone();
        m.reduce_add_scalar_mul_slice_assign(&mut acc, scalar, &b);
