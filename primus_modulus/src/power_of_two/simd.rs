...
// ===========================================================================
// SIMD slice kernels for PowOf2Modulus.
//
// Every operation is `op & mask` where `mask = 2^k - 1`. The `BitAnd`
// bound on `SimdArray` enables `&` on generic `Simd<T, N>`.
// ===========================================================================

#[inline]
pub fn reduce_once_slice_assign<T: SimdUnsignedInteger, const N: usize>(mask: T, values: &mut [T])
where
    Simd<T, N>: SimdArray<T, N>,
{
    let m = Simd::<T, N>::splat(mask);
...
#[inline]
pub fn reduce_neg_slice_assign<T: SimdUnsignedInteger, const N: usize>(mask: T, values: &mut [T])
where
    Simd<T, N>: SimdArray<T, N>,
{
    let m = Simd::<T, N>::splat(mask);
    ...
    for chunk in chunks {
        let v = Simd::from_array(*chunk);
        *chunk = (v.wrapping_neg() & m).to_array();
    }
    ...
    Simd<T, N>: SimdArray<T, N>,
{
    ...
    for (i, o) in in_chunks.iter().zip(out_chunks) {
        let v = Simd::from_array(*i);
        *o = (v.wrapping_neg() & m).to_array();
    }

    for (&i, o) in in_rem.iter().zip(out_rem) {
        *o = i.wrapping_neg() & mask;
    }
}
...
#[inline]
pub fn reduce_dot_product<T: SimdUnsignedInteger, const N: usize>(mask: T, a: &[T], b: &[T]) -> T
where
    Simd<T, N>: SimdArray<T, N>,
{
    ...
    Simd<T, N>: SimdArray<T, N>,
{
    ...
    for chunk in chunks {
        let v = Simd::from_array(*chunk);
        *chunk = ((v * s) & m).to_array();
    }
    for v in rem {
        *v = v.wrapping_mul(scalar) & mask;
    }
}
    ...
    Simd<T, N>: SimdArray<T, N>,
{
    ...
    for (ac, bc) in acc_chunks.iter_mut().zip(b_chunks) {
        ...
        *ac = ((acc_v + s * b_v) & m).to_array();
    }
    for (acc, &b) in acc_rem.iter_mut().zip(b_rem) {
        *acc = acc.wrapping_add(scalar.wrapping_mul(b)) & mask;
    }
}
    ...
    Simd<T, N>: SimdArray<T, N>,
{
    ...
    for (((ac, bc), cc), oc) in a_chunks.iter().zip(b_chunks).zip(c_chunks).zip(o_chunks) {
        ...
        *oc = ((av * bv + cv) & m).to_array();
    }
    for (((&a, &b), &c), o) in a_rem.iter().zip(b_rem).zip(c_rem).zip(o_rem) {
        *o = a.wrapping_mul(b).wrapping_add(c) & mask;
    }
}
    ...
    Simd<T, N>: SimdArray<T, N>,
{
    ...
    for ((bc, cc), oc) in b_chunks.iter().zip(c_chunks).zip(o_chunks) {
        let bv = Simd::from_array(*bc);
        let cv = Simd::from_array(*cc);
        *oc = ((s * bv + cv) & m).to_array();
    }
    for ((&b, &c), o) in b_rem.iter().zip(c_rem).zip(o_rem) {
        *o = scalar.wrapping_mul(b).wrapping_add(c) & mask;
    }
}
