...
macro_rules! pow2_ext_scalar {
    ($($t:ty),*) => {
        $(
            ...
            impl_lazy_mul_slice_delegates!(impl LazyReduceMulSlice<$t> for PowOf2Modulus<$t>);

            impl_lazy_mul_slice_delegates!(impl LazyReduceMulAddSlice<$t> for PowOf2Modulus<$t>);
        )*
    };
}
